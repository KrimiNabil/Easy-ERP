...
const compile = async function (fileName, data) {
    try {
        const filePath = path.join(process.cwd(), 'backend/payablePDFs/invoicePDF/', `${fileName}.hbs`);
...
async function generatePDF(doc) {
    try {
        ...
        console.log(plainDoc.invoiceID);

        await page.setContent(content);
        // Change the directory path here to save PDFs to the new location
        const pdfDirectory = '../invoicePDF/PDFs';
        const pdfPath = path.join(__dirname, pdfDirectory, `${plainDoc.invoiceID}.pdf`);
        ...
        await browser.close();
        console.log(`PDF generated successfully: ${plainDoc.invoiceID}.pdf`);
        // Return the relative URL for storage
        const relativePath = path.join(__dirname,pdfDirectory, `${plainDoc.invoiceID}.pdf`);
        return relativePath.replace(/\\/g, '/'); // Ensure consistent forward slashes for URLs
    } catch (error) {
