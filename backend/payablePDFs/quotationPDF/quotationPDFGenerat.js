...
const compile = async function (fileName, data) {
    try {
        const filePath = path.join(process.cwd(), 'backend/payablePDFs/quotationPDF/', `${fileName}.hbs`);
...
async function generatePDF(doc) {
    try {
        ...
        const pdfDirectory = '../quotationPDF/PDFs/';
        const pdfPath = path.join(__dirname, pdfDirectory, `${plainDoc.quotationNumber}.pdf`);
        ...
        await browser.close();
        console.log(`PDF generated successfully: ${plainDoc.quotationNumber}.pdf`);
        
         // Return the relative URL for storage
         const relativePath = path.join(__dirname,pdfDirectory, `${plainDoc.quotationNumber}.pdf`);
